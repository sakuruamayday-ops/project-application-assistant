...
建立企业知识产权事实底稿并判断其申报证据价值。FTO与侵权风险调用 `patent-fto-analysis`，权利要求技术范围调用 `patent-claim-analysis`，申请方向和文件分别调用布局及撰写技能。

## 流程

1. 锚定权利人主体和统计截止日期。
2. 专利先调用 `patent-data-foundation`，统一申请号、公开号、授权号、类型和法律状态。
