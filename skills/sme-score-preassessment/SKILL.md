...
用户要求前期培育报告、方向选择、产品定位或未来专利培育时，先完整读取 [前期培育报告方向卡先行模板](references/direction-card-first-template.md)。必须先分别列出企业现有方向、用户给定方向和建议新增方向，再逐项说明好处、坏处、培育建议和专利布局；完成比较与推荐后，才进入政策适配和申报成熟度。
...
6. 给出差距行动，按硬门槛、数据冲突、质量评价输入缺口、材料一致性排序。
7. 需要后期材料体检时交给 `sme-development-projects`，避免把预评估写成正式评审结论。
