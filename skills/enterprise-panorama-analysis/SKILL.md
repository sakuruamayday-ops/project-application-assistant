...
1. 读取版本A规范、`assets/sales-style.css` 和随包母版样张 `assets/sales-master-reference.pdf`；母版只用于视觉参数与第一至第六章、原第八至第九章的结构参照，第七章必须按现行规范替换为“经营基础与资料核验”，不得沿用样张中的财务估算标题或内容。使用宿主平台现有网页与PDF能力检查决策层级、证据可追溯性、A4分页和内容保真，不要求本技能包重复提供通用PDF、Word或Excel Skill。
...
5. 抽查首页、第七章“经营基础与资料核验”、新增第八至第十一章、顺延后的第十二至第十三章、最长风险表和最终总结页；检查新增或替换章节与母版的标题字号、正文与表格字号、页边距、单元格内边距、表头颜色、边框、页脚和分页是否完全一致。
