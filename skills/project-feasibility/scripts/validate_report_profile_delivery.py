...
def _pdf_font_is_self_contained(document: Any, xref: int, subtype: str) -> bool:
    """Accept embedded font files and PDF Type3 glyph programs.

    LibreOffice emits some CJK variable fonts as Type3 subsets. PyMuPDF cannot
    extract those as a font file, but their CharProcs streams contain the glyph
    outlines in the PDF itself, so they do not depend on a host-installed font.
    """
    try:
        extracted = document.extract_font(xref)
        if isinstance(extracted[3], bytes) and len(extracted[3]) > 0:
            return True
    except Exception:
        pass
    if subtype.casefold() != "type3":
        return False
    try:
        value_type, value = document.xref_get_key(xref, "CharProcs")
        if value_type != "dict":
            return False
        stream_xrefs = [int(item) for item in re.findall(r"\b(\d+)\s+0\s+R\b", value)]
        return bool(stream_xrefs) and all(
            document.xref_is_stream(item) and len(document.xref_stream(item)) > 0
            for item in stream_xrefs
        )
    except Exception:
        return False
    ...
    try:
        for page_number, page in enumerate(document, start=1):
            ...
            for row in page.get_fonts(full=True):
                xref, subtype, base_font = int(row[0]), str(row[2]), str(row[3])
                embedded = _pdf_font_is_self_contained(document, xref, subtype)
                fonts.append((_font_key(base_font), embedded))
            used: set[str] = set()
                ...
                for line in block.get("lines", []):
                    for span in line.get("spans", []):
                        ...
                        if not re.search(r"[\u3400-\u9fff]", text):
                            continue
                        used.add(_font_key(str(span.get("font") or "")))
    ...
    *,
    ...
    if pdf:
        try:
            ...
            errors.extend(_pdf_cjk_font_errors(pdf))
        except Exception as exc:
