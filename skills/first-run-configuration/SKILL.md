...
1. 首次运行收集默认政策地区和用户选择启用的供应商能力；团队知识配置通过门户的一段完整安装指令完成。
2. 团队知识服务使用当前登录用户的个人Bearer Token。Token只写入用户自己的WorkBuddy MCP配置，不得写入公共代码、能力报告、普通日志或最终回复。
...
11. 升级前备份原插件目录和用户MCP配置；覆盖新版后只替换`jiaotang-kb`条目，保留其他MCP。失败时恢复两份备份并报告失败阶段。
