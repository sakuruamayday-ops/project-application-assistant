...
三首项目按“公示—认定—奖励—目录退出”分别记录状态事件，不以后序状态删除前序证据。没有明确目录退出原文时不得推断退出。无法取得具体产品名称时先复用本轮用户材料，并按三首名单协议通过可用官方原文和附件有限补查；仍未取得时只返回企业、项目、年度、来源和缺失材料，不重复索要已提供文件，也不生成“未展开产品”等占位名称。
