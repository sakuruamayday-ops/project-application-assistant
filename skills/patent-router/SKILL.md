...
用户只问一个拟申请专利是否适配企业、产品或政府项目时，不启动完整专利案卷、企业画像或项目预评估。默认只回答四项：企业与产品或项目场景的技术关联、近似专利检索结果、基于现有技术差异的授权前景、提高区分度和授权可能性的修改建议。高企、专精特新、小巨人、知识产权、科技创新、产业化、质量品牌或其他项目名称只改变适配依据，不扩大回答范围。

除非用户询问权利状态、当期申报使用条件或明确要求完整项目评估，不主动追加“拟申请尚未授权”“不能作为已授权知识产权使用”等常识性提醒。用户明确要求专利布局、查新报告、申请文件、FTO、预审推荐或正式交付时，再进入对应完整流程。
...
1. `P1 检索与法律分析`
   ...
   - 中国专利首轮发现优先使用国家知识产权局公布公告适配器 `scripts/cnipa_epub_search.py`。只发送脱敏短技术词；题名、公开号和结果页摘要统一标记为 `discovery`，在逐项核对公开日、官方全文、权利要求、附图和法律状态前，`prior_art_eligible` 必须为 `false`。
   - Google Patents 不作为中国专利首轮默认入口，仅用于外国文献、同族、引证、官方入口暂不可用时的候选补充或全文补全；补充结果仍须回到对应法域官方原文。提供器顺序、安全边界和降级规则见 `references/cnipa-epub-provider.md`。
...
2. `P2 专利挖掘与交底策略`
   ...
   - 用户要求专利布局、年度 IP 规划或政府项目申报前知识产权规划时，完整读取 `references/patent-layout-planning.md`，形成核心、外围、工艺控制、应用和防御公开的组合及节奏；不得输出授权概率。
...
3. `P3 预审通道推荐`
   ...
   - 申请人注册地不作为默认反问项：根据用户给出的完整企业名称或统一社会信用代码，按 [企业数据源有限降级协议](../_runtime/enterprise-source-fallback.md) 取得登记住所并记录查询日期；主体简称先做实体识别。
...
- 国知局公布公告入口当前使用 HTTP。不得把未公开交底、整段权利要求、实施例参数、客户名称、个人信息或商业秘密作为检索词；页面要求验证码或人工交互时停止自动化，不绕过验证。
...
- 国知局适配器离线解析、类型复选框、跨词去重、证据降级、敏感查询拒绝、失败重试和零结果语义。
