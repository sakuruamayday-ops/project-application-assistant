...
class BaseSchemaValidator:
                        ...
                        if id_value is not None:
                            if scope == "global":
                                if id_value in global_ids:
                                    ...
                                    errors.append(
                                        f"  {xml_file.relative_to(self.unpacked_dir)}: "
                                        f"第 {elem.sourceline} 行: <{tag}> 中的全局 ID '{id_value}' "
                                ...
                                if id_value in file_ids[key]:
                                    ...
                                    errors.append(
                                        f"  {xml_file.relative_to(self.unpacked_dir)}: "
                                        f"第 {elem.sourceline} 行: <{tag}> 中重复的 {attr_name}='{id_value}' "
            ...
            try:
                ...
                if broken_refs:
                    ...
                    for broken_ref, line_num in broken_refs:
                        errors.append(
                            f"  {rel_path}: 第 {line_num} 行: 对 {broken_ref} 的损坏引用"
                        )
    ...
    def validate_content_types(self):
            ...
            for xml_file in self.xml_files:
                ...
                try:
                    ...
                    if root_name in declarable_roots and path_str not in declared_parts:
                        errors.append(
                            f"  {path_str}: 具有 <{root_name}> 根元素的文件未在 [Content_Types].xml 中声明"
                        )
            ...
            for file_path in all_files:
                ...
                if extension and extension not in declared_extensions:
                    if extension in media_extensions:
                        ...
                        errors.append(
                            f'  {relative_path}: 扩展名为 \'{extension}\' 的文件未在 [Content_Types].xml 中声明'
                        )
        ...
        if errors:
            ...
            if self.verbose:
                print("通过 - 所有内容文件都已在 [Content_Types].xml 中正确声明")
            return True
    ...
    def _get_original_file_errors(self, xml_file):
        ...
        with PersistentTemporaryDirectory(prefix="schema_original_") as temp_dir:
