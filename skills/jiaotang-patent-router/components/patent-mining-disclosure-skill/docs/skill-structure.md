...
**交付留档**：凡写入用户产出目录的交底书定稿（**含首次 Step 7 与迭代**），均写入 `vN-说明-YYYYMMDDHHMMSS/` 版本目录；主名为 **`{案件名}_{YYYYMMDDHHmmss}.md`** 或同名 **`.docx` / `.pdf`**（见 **`prompts/versioned_output.md`** 与 **`prompts/disclosure_builder.md` §7.6**）。旧稿保留在旧版本目录。交付格式由用户选择；Word 图示可选 PNG、生图提示词或自动生图。迭代时在案件根目录维护 **`版本索引.md`** 与 **`交底书修订对话记录.md`**（**`tools/iteration_dialog_log.py`** 或手工）。流程见 **`prompts/iteration_context.md`**。
