...
Step 8  内部自检（prompts/disclosure_self_check.md）→ 修订后交付
        ↓
交付   用户产出根目录下创建 `vN-说明-时间戳/`，其中写入 .md / .docx / .pdf：**案件名 + 本地时间戳**（`versioned_output.md` 与 §7.6），**含首次定稿与每次迭代**，勿默认覆盖；Word 图示按用户选择为 PNG、生图提示词或自动生图
        ↓
持续   意图为改已有稿时：补充材料（merger.md）/ 对话纠正（correction_handler.md）；规则同上；案件根目录 **`版本索引.md`** 更新，**`交底书修订对话记录.md`** 逐条追加（含时间、用户说明摘要），见 `iteration_context.md`
```
...
- 交底书定稿须由用户选择交付 Markdown 或 Word；所有交付文件写入 `vN-说明-YYYYMMDDHHMMSS/` 版本目录，文件名 **`{案件名}_{YYYYMMDDHHmmss}`**（`versioned_output.md` 与 §7.6，含首次与迭代）。若交付 Word，图示再由用户选择：PNG 模式由 `tools/mermaid_render.py` 渲染并嵌入系统框图 / 流程图；生图提示词模式用 `--diagram-mode prompt` 输出专利附图风格提示词；自动生图模式用 `--diagram-mode image-api` 调用用户配置的图片接口（见 `tools/README.md`）。
