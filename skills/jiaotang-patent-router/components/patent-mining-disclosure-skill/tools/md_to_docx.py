...
def _find_following_hidden_image_comment(
    lines: list[str],
    start: int,
) -> tuple[int, str, str] | None:
    """查找当前位置后跳过空行的隐藏图片注释，返回 (行号, alt, src)。"""
    j = start
    ...
    if not cm:
        return None
    return j, cm.group(1), cm.group(2).strip()
    ...
    while i < len(lines):
        ...
        if line.strip().startswith("```"):
            ...
            if fence_lang.lower() == "mermaid":
                hidden = _find_following_hidden_image_comment(lines, i)
                if hidden:
                    j, alt, src = hidden
                    if _is_diagram_image(alt, src):
                        ipath = _resolve_image_path(src, base_dir)
                        if ipath:
                            _embed_from_image_ref(
                                alt,
                                src,
