...
为保证基础页面不溢出：执行判断不超过5项；KPI 卡片不超过4项；总览指标行不超过8行；每个专题事实不超过4项、动作不超过6项、专题表格不超过8行；风险地图不超过8项；补充计算不超过10项；政策不超过5项；月度指标不超过5项。确定性指标不受补充计算数量限制，来源表自动增加续页。十四项风险闸门都要检查，但报告只汇总最重要的8项风险链。
