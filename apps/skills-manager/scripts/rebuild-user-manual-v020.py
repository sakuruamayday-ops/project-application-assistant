...
def build_scan_page(document: Document) -> None:
    ...
    add_callout(
        ...
        "不读取项目正文、客户资料、聊天记录或用户文档。正式平台扫描只识别 WorkBuddy。",
        fill=LIGHT_BLUE,
    )
    ...
    add_numbered_steps(
        ...
        [
            ...
            ("选择产物。 ", "根据宿主能力选择通用版或 WorkBuddy 跨平台版。"),
    ...
    add_heading(document, "两个正式产物的当前动作", 2)
    add_table(
        document,
        ["平台", "识别入口", "管理器动作", "用户最终动作"],
        [
            ["WorkBuddy", "应用安装位置", "准备并验签通用插件市场包", "在 WorkBuddy 应用内添加市场、安装并启用"],
            ["通用版", "标准 Skills 入口", "下载并验签通用 Skills 包", "按宿主标准入口导入并新建会话复验"],
        ],
        [1450, 1800, 2800, 3310],
        font_size=8.9,
        centered_columns={0},
