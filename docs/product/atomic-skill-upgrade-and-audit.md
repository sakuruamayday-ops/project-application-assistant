...
内部候选说明仍通过 `--release-notes` 保留验收状态；GitHub、门户和客户端更新源必须
共用 `--public-release-notes` 指定的状态中立说明。公开说明中如果仍出现“候选说明”、
“正式版本仍为”等只在提升前成立的措辞，发布命令会在任何对外写入前拒绝执行；
提升后还会逐字回读客户端更新源的 `releaseNotes`。
