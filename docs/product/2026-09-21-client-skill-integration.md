...
用户已授权本地签名候选及隔离验收。源提交 `0675c0a` 的 13 项签名前检查全部通过，51 项技能逐项签名、真实 ZIP 验签、隔离原子安装和安装后只读验证通过；源码、发布与安装内容一致。签名候选摘要见 [signed-candidate-20260921.json](evidence/signed-candidate-20260921.json)。未对外发布或替换正式安装。

客户端 V0.4.8 的技能投影从此已验签包的隔离安装目录生成，51 项技能和 567 个投影文件，沿用原正式信任密钥。App 内真实 DeepSeek 调用签名 DOCX 提取、review11 原生升级与启动失败恢复均已通过；review12 最终包资源验签及真实续聊通过，源技能内容不变。原有源码测试结果不代表全部技能业务均已通过。
