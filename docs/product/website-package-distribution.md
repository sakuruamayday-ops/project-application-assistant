# 网站安装包分发策略

状态：正式执行

更新日期：2026 年 7 月 28 日

## 产品入口

焦糖不再把原生 Skills Manager 或 PWA 管理器作为正式入口。知识门户的
“Skills → 版本与下载”是唯一正式分发面，负责展示当前版本、SHA-256、发布说明、
两个正式包及其下载按钮。

## 包型

| 包型 | 内容 | 用户选择 |
|---|---|---|
| 通用 Skills 包 | 49 项正式 Skills 与各自签名材料 | 宿主支持标准 Agent Skills，但不需要焦糖平台 Hooks 时使用 |
| WorkBuddy 跨平台包 | 同一套 Skills，加 WorkBuddy Hooks、插件描述和 MCP 声明 | 在 macOS 或 Windows WorkBuddy 中使用 |

WorkBuddy 包已经包含通用 Skills。用户只安装一个包，不叠加安装。

## 当前发布矩阵

| 平台 | 官方能力基线 | 网站状态 |
|---|---|---|
| 通用 Agent Skills | `SKILL.md` 目录或导入机制 | 正式可下载 |
| WorkBuddy | 本地插件市场、Hooks、MCP | macOS 与 Windows 共用一个跨平台包 |

正式发布矩阵固定为以上两个产物，不增加其他平台专用版本、占位 ZIP 或适配状态卡片。

## 版本号授权

- 正式版本号由主人在发布范围确定后明确指定；开发、合并和预发布准备不得自行递增版本号。
- 已正式发布的版本号、Git 标签、包 SHA-256 和签名记录不可覆盖。
- 指定版本若已存在，发布流程必须停止并报告冲突，等待主人指定尚未使用的版本。
- GitHub、门户和本机安装必须使用同一份签名事务清单；同一版本只允许一个任务持有
  发布租约，其他任务自动进入只读监控。
- 租约过期后的接管只允许继续完全相同的事务哈希；同版本换包、换提交或单独补发
  某一通道均被阻断，必须改发新的补丁版本。

## 双产物发布门禁

两个正式包必须同时满足：

1. 官方文档明确支持所使用的 Skills、Hooks、插件或 MCP 机制。
2. 包结构、配置字段和版本范围有固定校验器。
3. 安装前展示写入位置、联网域名、凭据类型和回滚方式。
4. 不使用外部 CLI、运行锁、动态命令或管理员权限绕过宿主安全边界。
5. WorkBuddy 包在对应真实宿主完成安装、升级、卸载、回滚和新会话触发验收。
6. 发布清单、逐文件 SHA-256 和发布者签名校验全部通过。
7. 正式提升前，以通用签名包原子升级发布机的 Codex Skills，并完成开发源、正式包、实际安装目录三方哈希比对和全量验签。
8. 发布事务状态必须依次证明 GitHub 暂存、门户暂存、本机安装、门户正式、GitHub
   正式和最终完成，任一步证据都写入同一事务审计链。

任一项未满足即阻断对应正式包发布，不生成第三种替代产物。

## 兼容与回滚

- 本次只改变网站分发入口，不修改现有用户机器上的任何 Skills、插件、Hooks、MCP、
  设备绑定或系统凭据。
- WorkBuddy 使用 V1.4.0 跨平台统一包；安装时先把完整市场持久保存到当前宿主的 `plugins/marketplaces/jiaotang`，再注册和启用，清理只覆盖下载与未注册中转文件。MCP 由签名插件清单内联声明，不再依赖插件根目录 `.mcp.json`，并继续将运行时知识库连接验证设为首次配置完成门槛，不改变 49 项业务技能数量。
- 原生客户端 v0.2.0 的 GitHub Release 不删除、不替换，只停止门户转发。
- 旧 `/skills-manager`、原生客户端下载和客户端手册地址统一跳转到
  `/skills#skills-downloads`。
