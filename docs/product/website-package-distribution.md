...
- 本次只改变网站分发入口，不修改现有用户机器上的任何 Skills、插件、Hooks、MCP、
  设备绑定或系统凭据。
- 网站不在文档中硬编码当前正式版本。门户从已发布 artifact 记录读取通用版和 WorkBuddy 版版本，并对 WorkBuddy ZIP 执行插件根 `.mcp.json` 能力检查。未通过能力检查、仍属候选或待实机验收时，网站必须显示“暂停新安装”，不得提供会复现已知故障的安装指引。
- 可安装的 WorkBuddy 包必须把插件根 `.mcp.json`、`plugin.json` 相对路径声明和运行时连接代码一并纳入签名文件集合。安装时先把完整市场持久保存到当前宿主目录，再注册和启用；清理只覆盖下载与未注册中转文件。
- 已绑定设备通过门户两步升级事务复用原设备身份、密钥、访问凭据与知识库连接；目标包验签、真实工具枚举和只读连接复核通过后才更新版本，失败时恢复升级前插件目录。
