...
<p align="center">
  <a href="docs/releases/V1.6.19.md"><img src="https://img.shields.io/badge/Release-V1.6.19-C9A760?style=for-the-badge" alt="Release V1.6.19"></a>
...
V1.6.19 完整保留财税报告计算与来源条目，超过单页容量时自动续页，按实际生成页数验收；继续保留确定性计算、专业交付有界收束和独立客户端同源技能投影。客户端与技能包分别版本化、分别验收，正式状态以 GitHub Latest 与受控发布回执为准。

### 当前版本功能简介

#### 一、本版本新增功能

- 任务沿用用户在当前任务选择的模型与推理档位，不固定模型、档位、搜索轮数或无条件多代理策略。
- 已授权且范围未变化的动作直接继续；高影响缺口只暂停依赖部分，能够独立完成的工作继续推进。
- 首次配置优先读取本轮真实能力，只在任务确实缺少必要能力时给出对应配置步骤。
- 套件清单声明的全部正式 Skills 统一从宿主提供的实际技能路径执行，不依赖特定第三方宿主变量。
- 签名、发布者指纹和内容完整性异常继续失败关闭；辅助偏好不可写时只降级受影响能力。
- 专业交付最多自动修正一次，仍不满足时形成待完善、等待用户或失败终态，不再无限续跑。
- 自动记录、自动归档与持久化授权分离，未获授权时不写入长期记忆或用户资料目录。
- 通用签名包与独立客户端内嵌投影使用同一有效技能清单，技能数量从验签清单派生。
...
| [V1.6.19 功能简介](docs/releases/V1.6.19.md) | 财税报告计算与来源完整保留、自动续页和实际页数验收 |
...
| 层级 | 当前值 | 说明 |
|---|---|---|
| 产品标签 | `V1.6.19` | 本次发布版本；GitHub、受控门户和本机活动技能按同一签名事务同步 |
| 组件版本 | `1.6.19` | 套件与 Python 组件版本；独立客户端另行版本化与验收 |
