...
1. 在门户点击“复制给 Agent”。
2. 把复制的文字粘贴到本地 Agent。
3. 门户向 Agent 提供一个 `jiaotang-agent-install/v1` HTTPS 安装说明。Agent 必须先展示安装器来源与 SHA-256、联网地址、本地改动、凭据保存方式和回滚方法。
4. Agent 读取公开安装器源码并核验 SHA-256，向用户说明风险；只有用户明确确认后，才执行说明中的 `execution.command`。
5. 协议命令在同一进程链中下载安装器，并依次完成完整性校验、设备登记、系统凭据库保存、首次签名验签和 `jiaotang-kb` stdio MCP 连接。
...
8. 所有安装结局统一输出结构化的 `jiaotang-agent-result/v1` 结果。Agent 应说明成功或失败、已完成阶段和下一步，不得显示安装码、API Token 或私钥。
9. WorkBuddy 写入新连接器后仍需用户在左侧“连接器”中找到 `jiaotang-kb` 并点击“启用”。安装器会返回 `activation_required=true`，并通过统一结果中的 `next_action` 给出操作提示。
10. 安装器会把最后一次结构化结果回传门户。管理员可在“成员管理”查看成功或失败状态，并进入成员详情查看失败阶段、用户提示和下一步。若 Agent 连安装说明都未读到，门户显示“未收到结果”。

支持 WorkBuddy 与具备本地命令执行、联网读取和用户配置写入能力的通用本地 Agent；支持 macOS 与 Windows。纯云端 Agent 不在此范围。
