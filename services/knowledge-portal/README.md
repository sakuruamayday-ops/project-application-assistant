...
- 登录密码使用 Argon2 哈希保存。
- 忘记密码时，访问 `/password/reset`，填写英文账号、该账号登记的公司全称和两次新密码。活动且具备登录资格的账号可自助重置，管理员账号使用同一规则；停用、删除及未授权账号不能借此恢复。公司名称不是身份所有权证明，此入口属于已确认采用的低保障恢复方式，知道账号及公司全称的人仍可能重置密码。
- 找回表单使用同站 Cookie 与 CSRF 校验，页面禁止缓存；同一账号或 IP 在30分钟内失败5次后暂停尝试。两次新密码须一致且至少9位。找回、管理员重置及登录后的主动改密均撤销该账号的网页登录和密码登录型客户端凭据，不撤销独立个人 MCP Token；现有客户端单设备登录的凭据替换策略保持不变。
...
`build_recognition_search_index.py` 同时重建 `recognition_text_rows` 与 SQLite FTS5 trigram 索引。全文索引为已认定记录和待核线索筛选主题证据候选，最后仍使用原始证据、原有 LIKE 条件、认定排除和排序分页；不会把索引词命中直接当作认定结论。查询词均不少于三个字符时启用，含短词或旧索引没有新表时继续原查询，避免漏掉“湿巾”等短词。

派生表使用自己的整数主键，另存来源类型与原始证据 ID，不依赖来源表隐式 rowid。SQLite 触发器维护企业主题证据、企业提及内容和文档标题的新增、修改及删除，同库事务回滚时同步撤回；原始资料仍是事实来源。重建需在索引候选中进行，候选验收与现有签名发布流程完成前不替换活动索引。只更新服务代码不会让旧活动索引自动获得加速效果。短词路径仍需单独衡量性能，不能将长词提速结果扩大为全部容量验收。
