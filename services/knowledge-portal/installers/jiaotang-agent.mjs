...
async function install(argumentsValue) {
  ...
  const targetScript = installedScriptPath(home);
  ...
  const configPath = configureHost(host, home, mcpServerConfiguration(targetScript));
  ...
  const activationRequired = host === "workbuddy";
...
async function main() {
  ...
  process.stderr.write(
    `jiaotang-agent ${VERSION}\n用法：install | serve | simulate-config\n`,
  );
  process.exitCode = 2;
}
